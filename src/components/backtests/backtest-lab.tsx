  ...
  return <div className="grid gap-6 xl:grid-cols-[.9fr_1.4fr]">
    <section className="panel h-fit xl:sticky xl:top-24">
      ...
      <div className="mt-6 grid gap-4">
        ...
        <label className="flex items-center gap-3 text-sm text-white/65"><input type="checkbox" checked={allowShort} onChange={(e) => setAllowShort(e.target.checked)} /> Allow short positions</label>
        ...
        <p className="text-xs leading-5 text-white/40">A result is saved only when the configured market-data provider returns at least 50 real candles. Zerion X1 does not fabricate historical performance.</p>
      </div>
    </section>

    <div className="space-y-6">
      {result?.metrics ? <ResultView result={result} /> : <section className="panel min-h-72 grid place-items-center text-center"><div><p className="text-5xl">⌁</p><h2 className="mt-4 text-2xl font-semibold">No completed simulation selected</h2><p className="mt-2 max-w-lg text-sm text-white/55">Choose a strategy and a connected historical data source to calculate trades, drawdown and risk-adjusted metrics.</p></div></section>}
      <section className="panel"><div className="panel-header"><div><p className="eyebrow">Persistent history</p><h2 className="mt-2 text-2xl font-semibold">Saved backtests</h2></div><span className="data-badge">{history.length}</span></div><div className="mt-5 space-y-3">{history.length === 0 ? <p className="text-sm text-white/55">Completed results will appear here after they are saved to your account.</p> : history.map((item) => <button key={item.id} onClick={() => setResult(item)} className="flex w-full items-center justify-between rounded-2xl border border-white/10 p-4 text-left hover:bg-white/5"><div><strong>{item.request.symbol}</strong><p className="mt-1 text-xs text-white/45">{item.request.timeframe} · {item.request.startDate} → {item.request.endDate}</p></div><div className="text-right"><span className="data-badge">{item.status}</span>{item.metrics ? <p className="mt-2 text-sm">{item.metrics.netProfitPct.toFixed(2)}%</p> : null}</div></button>)}</div></section>
    </div>
  </div>;
}
...
function ResultView({ result }: { result: BacktestResult }) {
  ...
  return <section className="space-y-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{stats.map(([label,value]) => <div key={label} className="luxury-stat panel"><span>{label}</span><strong className="text-xl">{value}</strong></div>)}</div><div className="panel"><div className="panel-header"><div><p className="eyebrow">Equity and drawdown</p><h2 className="mt-2 text-2xl font-semibold">Performance curve</h2></div><span className="data-badge">real candles</span></div><svg viewBox="0 0 100 100" className="mt-6 h-64 w-full overflow-visible" preserveAspectRatio="none"><defs><linearGradient id="zip3-equity" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#e8c887" stopOpacity=".42"/><stop offset="1" stopColor="#e8c887" stopOpacity="0"/></linearGradient></defs><polyline points={`0,100 ${points} 100,100`} fill="url(#zip3-equity)" stroke="none"/><polyline points={points} fill="none" stroke="#e8c887" strokeWidth="1.2" vectorEffect="non-scaling-stroke"/></svg></div><div className="panel overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="text-white/45"><tr><th className="p-3">Side</th><th className="p-3">Entry</th><th className="p-3">Exit</th><th className="p-3">Qty</th><th className="p-3">Fees</th><th className="p-3">P&L</th><th className="p-3">Reason</th></tr></thead><tbody>{result.trades.slice(0,50).map((trade)=><tr key={trade.id} className="border-t border-white/8"><td className="p-3 uppercase">{trade.side}</td><td className="p-3">{trade.entryPrice.toFixed(2)}</td><td className="p-3">{trade.exitPrice.toFixed(2)}</td><td className="p-3">{trade.quantity}</td><td className="p-3">{trade.fees.toFixed(2)}</td><td className="p-3">{trade.pnl.toFixed(2)}</td><td className="p-3 text-white/55">{trade.exitReason}</td></tr>)}</tbody></table></div>{result.warnings.length ? <div className="panel"><h3 className="font-semibold">Methodology warnings</h3><ul className="mt-3 space-y-2 text-sm text-white/55">{result.warnings.map((warning)=><li key={warning}>• {warning}</li>)}</ul></div> : null}</section>;
}
