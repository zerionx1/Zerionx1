...
export function StrategyWorkspaceTabs({ strategies }: { strategies: StrategyDefinition[] }) {
  ...
  return <div className="space-y-8">
    ...
    <section id="archived"><div className="mb-4 flex items-end justify-between"><div><p className="eyebrow">Strategy archive</p><h2 className="mt-2 text-2xl font-semibold">Archived</h2></div><span className="data-badge">{archived.length}</span></div>{archived.length ? <StrategyList items={archived} /> : <p className="panel text-sm text-[#2F2A25]">Archived strategies stay available here with their versions and historical backtests.</p>}</section>
  </div>;
}

function Empty({ title, action, href }: { title: string; action: string; href: string }) {
  return <div className="panel"><h3 className="text-xl font-semibold">{title}</h3><p className="mt-2 text-sm text-[#2F2A25]">Start from a risk-managed template or build your own rule graph.</p><Link href={href} className="mt-5 inline-flex rounded-full border border-[#E6D8C3] px-5 py-2 text-sm text-[#2F2A25]">{action}</Link></div>;
}
