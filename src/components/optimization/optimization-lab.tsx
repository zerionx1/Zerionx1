...
export function OptimizationLab({ strategies }: { strategies: StrategyDefinition[] }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[.8fr_1.4fr]"><section className="panel h-fit"><p className="eyebrow">Walk-forward preparation</p><h2 className="mt-2 text-2xl font-semibold">Parameter search space</h2><p className="mt-3 text-sm leading-6 text-white/55">Generate controlled candidate sets before historical execution. Invalid combinations are rejected instead of silently tested.</p><div className="mt-6 grid gap-4"><label className="grid gap-2 text-sm text-white/55">Strategy<select className="luxury-input" value={strategyId} onChange={(e)=>setStrategyId(e.target.value)}>{strategies.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><Field label="Fast EMA values" value={fastValues} setValue={setFastValues}/><Field label="Slow EMA values" value={slowValues} setValue={setSlowValues}/><Field label="ATR stop multiples" value={stops} setValue={setStops}/><Field label="Risk-reward targets" value={targets} setValue={setTargets}/><Button disabled={!strategyId} onClick={()=>setPlanned(true)}>Generate candidates</Button></div></section><section className="space-y-5"><div className="grid gap-3 sm:grid-cols-3"><div className="panel luxury-stat"><span>Total combinations</span><strong>{candidates.length}</strong></div><div className="panel luxury-stat"><span>Valid candidates</span><strong>{valid.length}</strong></div><div className="panel luxury-stat"><span>Rejected</span><strong>{candidates.length-valid.length}</strong></div></div><div className="panel"><div className="panel-header"><div><p className="eyebrow">Ranked planning output</p><h2 className="mt-2 text-2xl font-semibold">Candidate matrix</h2></div><span className="data-badge">No fabricated returns</span></div><p className="mt-3 text-sm text-white/55">The score below ranks structural efficiency only. Actual performance is calculated later using real historical candles and out-of-sample validation.</p><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead className="text-white/45"><tr><th className="p-3">Fast</th><th className="p-3">Slow</th><th className="p-3">ATR stop</th><th className="p-3">Target R</th><th className="p-3">Structural score</th><th className="p-3">Status</th></tr></thead><tbody>{candidates.slice(0,80).map((item,index)=><tr key={`${item.fast}-${item.slow}-${item.stop}-${item.target}-${index}`} className="border-t border-white/8"><td className="p-3">{item.fast}</td><td className="p-3">{item.slow}</td><td className="p-3">{item.stop}</td><td className="p-3">{item.target}</td><td className="p-3">{item.score}</td><td className="p-3"><span className="data-badge">{item.status}</span></td></tr>)}</tbody></table>{!planned ? <p className="py-16 text-center text-white/45">Generate a bounded search space to begin.</p> : null}</div></div></section></div>;
}
function Field({label,value,setValue}:{label:string;value:string;setValue:(value:string)=>void}){return <label className="grid gap-2 text-sm text-white/55">{label}<input className="luxury-input" value={value} onChange={(e)=>setValue(e.target.value)} /></label>}
