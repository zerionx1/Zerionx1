  ...
  if (!coinDcxFallbackTimer) {
    coinDcxFallbackTimer = setInterval(() => {
      void (async () => {
        ...
        try {
          // Provider-backed fallback only.
          // CoinDCX ticker is generated by CoinDCX; no synthetic movement.
          const ticker = await getCoinDcxTicker();
          ...
          providers.coindcx.lastError =
            error instanceof Error
              ? `CoinDCX fallback ticker failed: ${error.message}`
              : "CoinDCX fallback ticker failed";
        } finally {
