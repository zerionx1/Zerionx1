...
export async function GET() {
  try {
    ...
    const opportunities = await select(
      "agent_opportunities",
      `status=eq.active&expires_at=gt.${encodeURIComponent(now)}&order=confidence.desc&limit=50`,
    );
    ...
    const missing = opportunities
      ...
      .map((row) => ({
        ...
        title: `${String(row.symbol)} opportunity`,
        body: `${String(row.direction)} · ${Number(row.confidence)}% confidence · ${String(row.reason)}`,
        priority: Number(row.confidence) >= 75 ? "high" : "normal",
      }));

    if (missing.length) await insert("user_notifications", missing);
